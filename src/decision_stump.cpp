...
int main(int argc, char** argv){
    ...
    //Train NBC
        decision_stump::DecisionStump<> stump(trainingFVs_combined,trainingGTVs,11,2);
    ...
    //Predict and Export Vectors
       for(int trainingIndex=0; trainingIndex<37; ++trainingIndex){
            ...
            stump.Classify(trainingFVs[trainingIndex],predictionOutputRow);
            std::stringstream prediction;
            prediction << "../data/Predictions_" << testNumber << "/DecisionStump_Prediction_Vectors/" << trainingImages[trainingIndex] << "_predictions.csv";
        ...
        for(int testingIndex=0; testingIndex<13; ++testingIndex){
            ...
            stump.Classify(testingFVs[testingIndex],predictionOutputRow);
            std::stringstream prediction;
            prediction << "../data/Predictions_" << testNumber << "/DecisionStump_Prediction_Vectors/" << testingImages[testingIndex] << "_predictions.csv";
            arma::mat predictionOutput(1,predictionOutputRow.n_cols);
            for(int i=0; i<predictionOutputRow.n_cols; ++i){
                predictionOutput(0,i) = predictionOutputRow(i);
              }
